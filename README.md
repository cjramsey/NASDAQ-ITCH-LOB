...
A high-performance C++23 implementation of a NASDAQ TotalView-ITCH 5.0 binary feed parser with full limit order book (LOB) reconstruction across all tracked instruments. Built to explore low-latency feed handling, memory-efficient design and market microstructure on real exchange data.


## Key Highlights

- End-to-end NASDAQ ITCH 5.0 parser and limit order book reconstruction on real exchange data  
- ~25 ns/message on 100MB datasets; sub-microsecond latency on full-day (~12GB) data  
...
Measured on a sample NASDAQ TotalView-ITCH 5.0 file on an Intel Core i7-1165G7 @ 2.80 GHz, 16GB RAM, compiled with `-O3 -march=native` and link-time optimization enabled.
...
The ~25 ns figure at 100MB reflects a working set that fits in L3 cache. As input size grows, the order index and per-symbol price maps together exceed cache capacity. At that point the dominant cost becomes random-access main memory latency on order lookups for executes, cancels, and deletes, not decode logic.
...
While a lock-free SPSC ring buffer was implemented to decouple parsing and processing, direct execution proved faster in this workload. The additional indirection and synchronisation overhead outweighed the benefits of concurrency, while direct processing benefited from improved cache locality.

This highlights a key trade-off in low-latency systems: additional concurrency does not always lead to better performance.
...
Unit tests cover `Orderbook`, `OrderbookManager` and `parser::` using Google Test, including: add bid/ask, partial cancel, full delete, partial and full execution, execution with price, and order replace. Ticker key conversion and timestamp parsing are also tested.
